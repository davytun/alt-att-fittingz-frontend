...
export const stylesApi = {
  ...
  uploadClientImages: async (clientId: string, data: UploadStyleImagesRequest): Promise<StyleImage[]> => {
    ...
    const API_URL = process.env.NEXT_PUBLIC_API_URL || "https://chosen-ophelia-daviwhizzy1-992ca0bc.koyeb.app/api/v1";

    console.log('Upload request:', { clientId, token: token ? 'present' : 'missing' });

    const response = await fetch(`${API_URL}/clients/${clientId}/styles/upload`, {
      method: "POST",
      headers: {
  ...
  uploadAdminImages: async (data: UploadStyleImagesRequest): Promise<StyleImage[]> => {
    // Get token from Zustand auth storage
    const authStorage = localStorage.getItem('auth-storage');
    let token = null;
    ...
    console.log('Admin upload request:', { token: token ? 'present' : 'missing' });
  ...
  getClientImages: (clientId: string, page = 1, pageSize = 10): Promise<StyleImagesResponse> =>
  ...
  // Update style image
  updateStyleImage: (clientId: string, imageId: string, data: UpdateStyleImageRequest): Promise<StyleImage> =>
    apiClient(`/clients/${clientId}/styles/${imageId}`, {
      method: "PATCH",
  ...
  // Delete style image
  deleteStyleImage: (clientId: string, imageId: string): Promise<{ message: string }> =>
    apiClient(`/clients/${clientId}/styles/${imageId}`, {
      method: "DELETE",
    }),
  ...
  deleteMultipleImages: (data: DeleteMultipleImagesRequest): Promise<DeleteMultipleImagesResponse> =>
