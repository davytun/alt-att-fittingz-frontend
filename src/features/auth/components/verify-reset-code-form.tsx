...
import { useVerifyResetCodeMutation } from "@/features/auth/hooks/use-auth-mutation";
import {
...
export function VerifyResetCodeForm() {
  ...
  const onSubmit = async (formData: VerifyResetCodeFormData) => {
    ...
    // Ensure we have a complete 6-digit code
    const code = resetCode.join("");
    if (code.length !== 6) {
      setServerError("Please enter a complete 6-digit code");
      return;
    }
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (pasteData.length === 6) {
      ...
      setServerError("Please paste a 6-digit code");
    }
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">
              We sent a 6-digit reset code to this email address
            </p>
          </div>
